...
export default async function TransactionTypeDetailPage({ params }: Params) {
  await ensureDb();
  const s = getSequelize();
  const TransactionType = s.model('TransactionType') as any;

  const row = await TransactionType.findByPk(params.id, { raw: true });
  if (!row) notFound();
  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <Link href="/admin/transaction-types" className="text-sm text-blue-600 hover:underline">← Back to Transaction Types</Link>
      </div>
